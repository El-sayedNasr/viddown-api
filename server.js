...
app.get('/', (_, res) => res.json({ ok: true, service: 'VidDown API v6' }));
...
app.get('/youtube', async (req, res) => {
  ...
  // args مختلفة تماماً لـ mp3 و mp4
  let args;

  if (format === 'mp3') {
    // ── MP3: -x فقط بدون --merge-output-format ──
    const tmpOut = path.join(os.tmpdir(), `yt_${vid}_${ts}.%(ext)s`);
    args = [
    ...
    // ── MP4: merge video+audio ──────────────────
    const tmpOut = path.join(os.tmpdir(), `yt_${vid}_${ts}.mp4`);
    args = [
      '--no-playlist', '--no-warnings',
  ...
  try {
    const outFile = await runYtDlp(args, vid, ts, ext);
    streamFileToResponse(outFile, fname, ext, res);
  } catch(e) {
...
//  runYtDlp
// ════════════════════════════════════════════════
function runYtDlp(args, vid, ts, ext) {
  return new Promise((resolve, reject) => {
    ...
    execFile('yt-dlp', args, { maxBuffer: 20 * 1024 * 1024 }, (err, stdout, stderr) => {
      ...
      if (err) {
        ...
        return reject(new Error('yt_dlp_failed: ' + msg.slice(0, 100)));
      }

      // ابحث عن الملف في /tmp
      const dir = os.tmpdir();
      try {
        const files = fs.readdirSync(dir)
          .filter(f => f.startsWith(`yt_${vid}_${ts}`) && (f.endsWith(`.${ext}`) || (ext === 'mp3' && f.endsWith('.mp3'))))
          .map(f => path.join(dir, f));

        if (files.length) return resolve(files[0]);

        // fallback: أي ملف بنفس الـ vid في آخر دقيقة
        ...
        if (fallback.length) return resolve(fallback[0].p);
      } catch(_) {}
...
// ════════════════════════════════════════════════
//  Stream file → response then delete
// ════════════════════════════════════════════════
function streamFileToResponse(filePath, fname, ext, res) {
...
function cleanTmp(vid, ts) {
  try {
    const dir = os.tmpdir();
    fs.readdirSync(dir)
      .filter(f => f.startsWith(`yt_${vid}_${ts}`))
  ...
  for (const base of ['https://api.cobalt.tools/', 'https://co.wuk.sh/']) {
    try {
      const data = await fetchJSON(base, {
        ...
        body: JSON.stringify({
          url, videoQuality: String(quality), audioFormat: 'mp3',
          filenameStyle: 'basic',
...
function fetchJSON(url, opts = {}, timeout = 12000) {
  return new Promise((resolve, reject) => {
    ...
    const req   = lib.request({
      ...
      path:     parsed.pathname + parsed.search,
      method,
      headers: { 'User-Agent': 'VidDown/6.0', 'Accept': 'application/json', ...headers },
    }, (res) => {
...
app.listen(PORT, () => console.log(`✅ VidDown v6 on port ${PORT}`));
