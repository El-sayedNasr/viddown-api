...
if (process.env.YT_COOKIES) {
  try { fs.writeFileSync(COOKIES_FILE, process.env.YT_COOKIES, 'utf8'); console.log('✅ cookies loaded'); }
  catch(e) { console.error('cookies error:', e.message); }
}
const cookiesArgs = () => fs.existsSync(COOKIES_FILE) ? ['--cookies', COOKIES_FILE] : [];

// ── Find yt-dlp ───────────────────────────────────
function getYtDlp() {
  for (const c of ['yt-dlp','/usr/bin/yt-dlp','/usr/local/bin/yt-dlp']) {
    try { if (fs.existsSync(c)) return c; } catch(_) {}
  }
  return 'yt-dlp';
}

app.get('/', (_, res) => res.json({ ok:true, service:'VidDown v10' }));
...
// ════════════════════════════════════════════════
// صيغ الصوت المدعومة
const AUDIO_FORMATS = ['mp3','m4a','ogg','flac','wav','aiff','opus'];
...
app.get('/youtube', async (req, res) => {
  ...
  if (isAudio) {
    // ── صوت: -x بدون --merge-output-format ────────
    const aq = ['320','256','192','128','96','64'].includes(quality)
  ...
  console.log(`[YT] vid=${vid} fmt=${format} q=${quality} audio=${isAudio}`);

  try {
    const outFile = await runYtDlp(bin, args, tmpBase, isAudio ? format : 'mp4');
...
app.get('/tiktok', async (req, res) => {
  ...
  try {
    const su = await getTikWMStream(url, format) || await tryCobalt(url,'1080',format);
    if (!su) return res.status(503).json({ error:'no_stream_found' });
    proxyStream(su, fname, format, res);
  } catch(e) { if (!res.headersSent) res.status(503).json({ error:e.message }); }
});

// ════════════════════════════════════════════════
//  runYtDlp
// ════════════════════════════════════════════════
function runYtDlp(bin, args, tmpBase, ext) {
  return new Promise((resolve, reject) => {
    ...
    execFile(bin, args, { maxBuffer: 20*1024*1024 }, (err, stdout, stderr) => {
      ...
      try {
        ...
        const exact = files.find(f => f.endsWith('.' + ext));
...
function streamFile(filePath, fname, format, res) {
  ...
  const mime = mimes[format] || 'application/octet-stream';
  ...
  s.pipe(res);
  s.on('close', () => { try { fs.unlinkSync(filePath); } catch(_) {} });
  s.on('error', () => { try { fs.unlinkSync(filePath); } catch(_) {} });
}

function cleanTmp(tmpBase) {
  try {
    const dir=path.dirname(tmpBase), pre=path.basename(tmpBase);
    fs.readdirSync(dir).filter(f=>f.startsWith(pre))
      .forEach(f=>{ try{fs.unlinkSync(path.join(dir,f));}catch(_){} });
  } catch(_) {}
}

function proxyStream(url, filename, format, res) {
  const mimes = { mp3:'audio/mpeg', m4a:'audio/mp4', ogg:'audio/ogg', flac:'audio/flac', wav:'audio/wav', mp4:'video/mp4' };
  const mime = mimes[format] || 'video/mp4';
  let parsed; try { parsed=new URL(url); } catch(_) { return res.status(400).end(); }
  const lib = parsed.protocol==='https:' ? https : http;
  lib.get({
    hostname:parsed.hostname, port:parsed.port||(parsed.protocol==='https:'?443:80),
    path:parsed.pathname+parsed.search, headers:{'User-Agent':'Mozilla/5.0','Accept':'*/*'}
  }, (up) => {
...
function fetchJSON(url, opts={}, timeout=12000) {
  return new Promise((resolve,reject) => {
    ...
    const req=lib.request({
      ...
      headers:{'User-Agent':'VidDown/10.0','Accept':'application/json',...headers},
    },(res)=>{
...
app.listen(PORT, ()=>console.log(`✅ VidDown v10 on port ${PORT}`));
